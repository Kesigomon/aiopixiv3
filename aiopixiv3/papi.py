...
class PixivAPI(BasePixivAPI):
    ...
    async def auth_requests_call(self, method, url, headers={}, params=None, data=None):
        self.require_auth()
        headers['Referer'] = 'http://spapi.pixiv.net/'
        headers['User-Agent'] = 'PixivIOSApp/5.8.7'
        headers['Authorization'] = 'Bearer %s' % self.access_token
        r = await self.requests_call(method, url, headers, params, data)
        r.encoding = 'utf-8'  # Manually set the encoding due to #11 #18 #26, thanks @Xdynix
        return r

    async def parse_result(self, req):
        try:
            return self.parse_json(await req.text())
        except Exception as e:
            raise PixivError("parse_json() error: %s" % (e), header=req.headers, body=await req.text())
    ...
    # 作品详细
    async def works(self, illust_id, include_sanity_level=False):
    ...
    async def me_favorite_works(self, page=1, per_page=50, publicity='public', image_sizes=['px_128x128', 'px_480mw', 'large']):
    ...
    async def search_works(self, query, page=1, per_page=30, mode='text',
        ...
        params = {
            ...
            'include_sanity_level': int(include_sanity_level),
            'image_sizes': ','.join(image_sizes),
        }
        r = await self.auth_requests_call('GET', url, params=params)
        return await self.parse_result(r)
